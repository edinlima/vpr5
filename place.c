...
static void my_initial_placement (enum e_pad_loc_type pad_loc_type,
            char *pad_loc_file, int *pins_on_block);
...
static int try_swap (float t, float *cost, float *bb_cost, float *timing_cost, 
		     ...
		     float *delay_cost, int only_io, int bfrom);

static int simple_swap (float t, float *cost, float *bb_cost, float *timing_cost, 
		     float rlim, int *pins_on_block,
		     int place_cost_type, float **old_region_occ_x, 
		     float **old_region_occ_y, int num_regions, boolean fixed_pins,
		     enum e_place_algorithm place_algorithm, float timing_tradeoff,
...
static int assess_swap (float delta_c, float t, float cost);
...
static int find_empty_neighbor (int x_from, int y_from, int *x_to, int *y_to, int type);

static void getCone(int *cone, int element, int *pins_on_block, int *tree);

static int find_cone(int *vetor, int element);

static void init_array(int *vetor, int val, int dim);

static int place_output(int *placed, int *output_block, int tsize);
/*****************************************************************************/
 ...
 * slightly more efficient.                                                */
 ...
 pins_on_block[INPAD] = 1;
  ...
  if (placer_opts.place_algorithm == NET_TIMING_DRIVEN_PLACE ||
     ...
     my_initial_placement (placer_opts.pad_loc_type, placer_opts.pad_loc_file, pins_on_block);
   ...
   if (placer_opts.place_algorithm == NET_TIMING_DRIVEN_PLACE ||
    ...
    for (inner_iter=0; inner_iter < move_lim; inner_iter++) {
      if (try_swap(t, &cost, &bb_cost, &timing_cost, 
	     ...
	     inverse_prev_timing_cost, &delay_cost, 0, -1) == 1) {
 ...
 * you get when you recompute from scratch.                       */
 ...
 for (inner_iter=0; inner_iter < move_lim; inner_iter++) {
   if (try_swap(t, &cost, &bb_cost, &timing_cost, 
	  ...
	  inverse_prev_bb_cost, inverse_prev_timing_cost, &delay_cost, 0, -1) == 1) {
     success_sum++;
     av_cost += cost;
     av_bb_cost += bb_cost;
     av_delay_cost += delay_cost;
...
/* Try one move per block.  Set t high so essentially all accepted. */
  if (place_algorithm == NET_TIMING_DRIVEN_PLACE ||
      ...
      for (i=0;i<move_lim;i++) { // 1.e30
         if (try_swap (1.0e30, cost_ptr, bb_cost_ptr, timing_cost_ptr, rlim,
     ...
     for (i=0;i<num_blocks;i++) { // 1.e30
        if (try_swap (0.0, cost_ptr, bb_cost_ptr, timing_cost_ptr, rlim, 
              pins_on_block, place_cost_type,
              old_region_occ_x, old_region_occ_y, num_regions, 
              fixed_pins, place_algorithm, timing_tradeoff,
	      inverse_prev_bb_cost, inverse_prev_timing_cost,
	      delay_cost_ptr, 0, i) == 1) 
        {
          num_accepted++; 
          av += *cost_ptr;
          sum_of_squares += *cost_ptr * (*cost_ptr);
...
/* Initial Temp = 20*std_dev. */
 ...
 if (num_accepted != move_lim) {
  ...
  if (place_algorithm == NET_TIMING_DRIVEN_PLACE ||
     ...
     return (0.035 * std_dev); 
  }
 //return (20. * std_dev); 
/* return (15.225523	); */
}

static int sort_from (boolean fixed_pins, int only_io)
{
 ...
 * doesn't change.                                                      */
 ...
 keep_switch = assess_swap (delta_c, t, *cost); 
...
/* 
  if (t > 0.025 && t < 0.5)
    factor = 1;
  else if (t <= 0.025 && t > 0.075 )
    factor = 3;
  else if (t <= 0.075 && t > 0.0075)
    factor = 2;
  else if (t <= 0.0075)
    factor = 1;
*/
  ...
  if(bfrom == -1)
   b_from = sort_from (fixed_pins, only_io);
  else
     b_from = bfrom;
...
/* Restore the region occupancies to their state before the move. */
    ...
    if(!keep_switch && (try_count < factor) && (block[b_from].type == CLB))
      find_to (x_from, y_from, block[b_from].type, rlim, &x_to, &y_to);
    //finded = 0;
  ...
  } while (!keep_switch && (try_count < factor) && (block[b_from].type == CLB));
...
/* Returns: 1 -> move accepted, 0 -> rejected. */ 
 ...
 if (t == 0.) 
    return(0);
 //printf("Custo %g delta_c %g variacao %g\n",cost,delta_c,delta_c/cost);
 ...
 fnum = my_frand();
 //fnum = 0.5;
 //printf("Fnum = %g delta_c = %g t = %g exp(-delta_c/t) = %g\n",fnum,delta_c,t,exp(-delta_c/t));
...
static void comp_td_costs (float *timing_cost, float *connection_delay_sum) {
  ...
  for (inet=0;inet<num_nets;inet++) {     /* for each net ... */
    if (is_global[inet] == FALSE) {    /* Do only if not global. */
      //printf("Coputando custo da net %d \n",inet);
      for (ipin=1; ipin<net[inet].num_pins; ipin++) {
      //printf("pino %d\n",ipin);
...
static void my_initial_placement (enum e_pad_loc_type pad_loc_type,
  ...
  int output_block[num_blocks]; // output blocks list
  int cone[num_blocks];  // cone model
  int tree[num_blocks];
  int placed[num_blocks];  // placed blocks list
  ...
  /* Initialize all positions at output_blocks[] with -1 */
  init_array(output_block,-1 ,num_blocks);
  init_array(placed,0,num_blocks);
  init_array(cone,-1 ,num_blocks);
 
  
  /* Place the outputs blocks around the periphery */

  int ind_ob; // output block indice
   ...
   if (pad_loc_type == USER) {
      ...
      ind_ob = place_output(placed, output_block, tsize);

     
   }    /* End randomly place OUTPUT blocks branch of if */
   ...
   for(i=0; i < ind_ob; i++){
      j = 1;
      init_array(cone, -1, num_blocks);
      init_array(tree, -1, num_blocks);
        
      getCone(cone, output_block[i], pins_on_block, tree);
    
     // printf("Listando cone\ttree de %d\n",cone[0]);
      //for(h=0; h<num_blocks; printf("%d\t%d\n",cone[h],tree[h++]));
      
      
      while(cone[j] != -1 && j < num_blocks){
       
         if(placed[cone[j]] != 1){
                  
               x = block[tree[cone[j]]].x;
               y = block[tree[cone[j]]].y;
               //printf("posicionando bloco %d do tipo %d\n",cone[j],block[cone[j]].type);
               
               //printf("buscando vizinho de %d %d\n",x,y);
               if(find_empty_neighbor(x,y,&x,&y,block[cone[j]].type)){
                  if (block[cone[j]].type == CLB) {    
                     clb[x][y].u.block = cone[j];
                     clb[x][y].occ = 1;
                  }else{                     
                     isubblk = clb[x][y].occ;
                     clb[x][y].u.io_blocks[isubblk] = cone[j];
                     clb[x][y].occ++;
                  }
                  block[cone[j]].x = x;
                  block[cone[j]].y = y;
                          
                  //printf("Posicionando bloco cone[%d] bloco %d em CLB[%d][%d]\n",j,cone[j],pos[choice].x,pos[choice].y);
                  placed[cone[j]] = 1;
               }else{
   ...
   for(i=0; i<num_blocks;i++){
      if(placed[i] != 1){
         clb[pos[choice].x][pos[choice].y].u.block = i;
   ...
   printf("Total de blocos %d posicionados %d\n",num_blocks,choice);
...
int is_connected (int b_from, int b_to, int *pins_on_block)
{
...
void getCone(int *cone, int element, int *pins_on_block, int *tree){

  int i,j, ipin,pin_net, num_pins ,id_net;
  ...
  while(cone[i] != -1 && i < num_blocks){
       num_pins = pins_on_block[block[cone[i]].type];
       for(ipin =0; ipin < num_pins; ipin++){
           if(is_opin(ipin))
              continue;
           
           id_net = block[cone[i]].nets[ipin]; // id da net conectada no respectivo pino

           if(id_net != OPEN){
              pin_net = net[id_net].num_pins;
              for(pin_net=0; pin_net < net[id_net].num_pins; pin_net++){

                  //printf("Avaliando bloco %d pino %d\n", net[id_net].blocks[pin_net],net[id_net].blk_pin[pin_net]);

                  /* Se o pino conectado a net for um DRIVER e o bloco não for igual ao pai e
                     ainda não estiver no cone - insira-o 
                  */
                  if((is_opin(net[id_net].blk_pin[pin_net]) || block[ net[id_net].blocks[pin_net]].type == INPAD ) 
                  //if((is_opin(net[id_net].blk_pin[pin_net]) ) 
                            && (cone[i] != net[id_net].blocks[pin_net])
                            && !find_cone(cone,net[id_net].blocks[pin_net]))
                  {
                     
                     cone[j] = net[id_net].blocks[pin_net];
                     tree[cone[j]] = cone[i];
                     //printf("Inserindo o bloco %d\n",cone[j]);
                     j++;
                     
                  }
                  

                  //printf("Bloco %d - Net %d conectada ao bloco %d pelo pino %d\n", cone[i],id_net,net[id_net].blocks[pin_net],net[id_net].blk_pin[pin_net]);
    ...
    //for(i =0; i < num_blocks; printf("Bloco %d\n",cone[i++]));
...
/*
* Find element in array vetor
*/
int find_cone(int *vetor, int element){
   int i = 0;
   while(vetor[i] != -1){
      if(vetor[i] == element)
         return 1;
 ...
 * Fill array with val 
*/
void init_array(int *vetor, int val, int dim){
   int i;
   for(i=0; i<dim; vetor[i++] = val);

   return;
}


int place_output(int *placed, int *output_block, int tsize){
   int i, iblk, choice, isubblk, ind_ob;
   int count = 0;
   ind_ob = 0;
   struct s_pos *pos;
   pos = (struct s_pos *) my_malloc(tsize*sizeof(struct s_pos));
   ...
   for (iblk=0;iblk<num_blocks;iblk++) {
      ...
      if (block[iblk].type  == OUTPAD) { //|| block[iblk].type  == INPAD) {
         choice = my_irand (count - 1); 
         isubblk = clb[pos[choice].x][pos[choice].y].occ;
         clb[pos[choice].x][pos[choice].y].u.io_blocks[isubblk] = iblk;
         clb[pos[choice].x][pos[choice].y].occ++;
         block[iblk].x = pos[choice].x;
         block[iblk].y = pos[choice].y;
         placed[iblk] = 1;
         //if(block[iblk].type  == OUTPAD)
            output_block[ind_ob++] = iblk;
   ...
   printf("Foram posicionados %d blocos de OUTPUT\n",ind_ob);
      ...
      if(delta > 0){
         ...
         for(i=-delta; i <= delta; i++){
            for(j = -delta; j<= delta; j++){
               newX = x_from + i;
               newY = y_from + j;            
               if(fabs(i) == delta || fabs(j) == delta){
                  if(type == CLB){
                  
                     if(newX > 0 && newX <= nx &&  newY > 0  && newY <= ny){
                  
                        if(clb[newX][newY].occ == 0){
                           *x_to = newX;
                           *y_to = newY;
                           return 1;
                        }
                     ...
                     if(((newX == 0 || newX == nx+1) && (newY > 0) && (newY < ny+1)) || 
                        ((newY == 0 || newY == ny+1) && (newX > 0) && (newX < nx+1))){
                  
                        if(clb[newX][newY].occ < io_rat){
                           *x_to = newX;
                           *y_to = newY;
                           return 1;
