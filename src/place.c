...
void
try_place (struct s_placer_opts placer_opts, struct s_annealing_sched
 ...
 * the place_cost_type is NONLINEAR_CONG.                                */
  ...
  /* initial_placement define posicao inicial - by ednaldo */
 ...
 * significant optimization).                                         */
  ...
  t_inicio = time(NULL); // add by Ednaldo to compute time of execution
  while (exit_crit (t, cost, annealing_sched) == 0)
	...
	{
	  if (try_swap
	      (t, &cost, rlim, pins_on_block, placer_opts.place_cost_type,
	       old_region_occ_x, old_region_occ_y, placer_opts.num_regions,
	       fixed_pins) == 1)
	    {
	      success_sum++;
 ...
 * you get when you recompute from scratch.                       */
  ...
  t_fim= time(NULL);  // add by Ednaldo to compute time of execution
  printf("Tempo de execucao = %f segundos.\n",difftime(t_fim, t_inicio));// add by Ednaldo to compute time of execution
...
static void
update_t (float *t, float std_dev, float rlim,
...
static int
try_swap (float t, float *cost, float rlim, int *pins_on_block,
	  int place_cost_type, float **old_region_occ_x,
	  float **old_region_occ_y, int num_regions, boolean fixed_pins)
{
  ...
  if (block[b_from].type == CLB)
    {
      ...
      if (clb[x_to][y_to].occ == 1)
	{			/* Occupied -- do a switch */
	...
	{			/* io block was selected for moving */
	  ...
	  if (b_to != EMPTY)
	    ...
	    {			/* Moved to an empty location */
	      ...
	      clb[x_to][y_to].occ++;
	      for (k = off_from; k < clb[x_from][y_from].occ - 1; k++)
		{		/* prevent gap  */
...
static void
find_to (int x_from, int y_from, int type, float rlim, int *x_to, int *y_to)
{
...
static void
initial_placement (enum e_pad_loc_type pad_loc_type, char *pad_loc_file)
{
